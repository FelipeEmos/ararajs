...
By default, all arara primitives manage their state internally. This means that they handle their state themselves and you don't need to pass any props/state to them.

arara aims to be very customizable and provides various ways to control a primitive or access internal state. Let's take a look at them. We'll use the [Dialog](/docs/primitives/dialog/) primitive as an example, but the same applies to all other primitives.
...
## Keyed context
There may be situations where you want to use nested instances of the same primitive. For example, multiple dialogs with multiple trigger buttons that are hard to separate in the template. For this case, arara allows you to pass a `contextId` to every primitive component to tell which context to use.
