...
image: https://arara.dev/primitives/resizable.jpg
---
...
<PackageInfo packageName="@arara/resizable" />
...
<Code code={`
npm install @arara/resizable
`} lang="bash" copyButton />

The resizable is also included in the main `arara` package under `arara/resizable`.

## Usage
<Code code={`
import Resizable from '@arara/resizable' // 'arara/resizable'
// or
// import { Root, Panel, Handle } from '@arara/resizable'
`} lang="tsx" copyButton />
...
arara itself doesn't provide a first party way to persist the layout of resizable panels. However, you can achieve this easily with something like the <Link href="https://primitives.solidjs.community/package/storage" newTab>`makePersisted`</Link> utility from Solid Primitives:
