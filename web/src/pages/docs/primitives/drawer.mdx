...
image: https://arara.dev/primitives/drawer.jpg
---
...
<PackageInfo packageName="@arara/drawer" />
...
<Code code={`
npm install @arara/drawer
`} lang="bash" copyButton />

The drawer is also included in the main `arara` package under `arara/drawer`.

## Usage
<Code code={`
import Drawer from '@arara/drawer' // 'arara/drawer'
// or
// import { Root, Trigger, ... } from '@arara/drawer'
`} lang="tsx" copyButton />
...
<Code code={`
[data-arara-dialog-content][data-transitioning] {
...
arara's tailwind plugin also provides a `arara-transitioning` modifier that you can use:

<Code code={`
<Drawer.Content
  class="
    arara-transitioning:transition-transform
    arara-transitioning:duration-500
  "
>
</Drawer.Content>
`} lang="tsx" copyButton />
...
<Drawer>
  {(props) => (
    <Drawer.Overlay
      class="fixed inset-0 z-40 arara-transitioning:transition-colors arara-transitioning:duration-500 arara-transitioning:ease-[cubic-bezier(0.32,0.72,0,1)]"
      style={{
...
arara's drawer can handle `height: auto` and transition between heights when the content changes. This is possible by using a `ResizeObserver` to detect height changes and apply a fixed height for the time of the transition.
...
You can disable drag on an element by giving it the `data-arara-no-drag` attribute. arara will ignore any drag events on elements with this attribute.
...
### Persistent content
To persist the content of the drawer even when it's unmounted, you can use arara's <Link href="/docs/utilities/persistent/">`createPersistent`</Link> utility. It will cache the content when the drawer is opened for the first time and preserve its state and HTML elements.
