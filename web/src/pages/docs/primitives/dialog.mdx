...
image: https://arara.dev/primitives/dialog.jpg
---
...
<PackageInfo packageName="@arara/dialog" />
...
<Code code={`
npm install @arara/dialog
`} lang="bash" copyButton />

The dialog is also included in the main `arara` package under `arara/dialog`.

## Usage
<Code code={`
import Dialog from '@arara/dialog' // 'arara/dialog'
// or
// import { Root, Trigger, ... } from '@arara/dialog'
`} lang="tsx" copyButton />
...
### Nested dialogs
Feel free to nest dialogs however you want. arara is aware of nested dialogs and only dismisses the top-most dialog when you press the escape key or click outside the dialog.
...
### Persistent content
To persist the content of the dialog even when it's unmounted, you can use arara's <Link href="/docs/utilities/persistent/">`createPersistent`</Link> utility. It will cache the content when the dialog is opened for the first time and preserve its state and HTML elements.
