...
image: https://arara.dev/primitives/accordion.jpg
---
...
<PackageInfo packageName="@arara/accordion" />
...
<Code code={`
npm install @arara/accordion
`} lang="bash" copyButton />

The accordion is also included in the main `arara` package under `arara/accordion`.

## Usage
<Code code={`
import Accordion from '@arara/accordion' // 'arara/accordion'
// or
// import { Root, Trigger, ... } from '@arara/accordion'
`} lang="tsx" copyButton />
...
### Animation
arara sets the `--arara-disclosure-content-height` and `--arara-disclosure-content-width` css properties on every accordion item content that make it possible to animate the height/width.

<Code code={`
[data-arara-accordion-content][data-collapsed] {
  animation: collapse 200ms linear;
}
[data-arara-accordion-content][data-expanded] {
